...
public class ConsultaIndividualSQLite {
    ...
    public String MetodoLogin(String nombreUsuario, String clave) {
        try {
            CrearBDSQLite manejadorBD = new CrearBDSQLite(contexto, "RegistroTecnoyomu", null, 1);
            SQLiteDatabase baseDatos = manejadorBD.getWritableDatabase();
            Cursor usuarios = baseDatos.rawQuery("SELECT * FROM usuario WHERE nombreUsuario='" + nombreUsuario + "' AND clave='" + clave + "'",
                    null);
            if (usuarios.moveToFirst()) {
                baseDatos.close();
                return usuarios.getString(usuarios.getColumnIndexOrThrow("rol"));
            } else {
                baseDatos.close();
                return null;
            }
        } catch (Exception e) {
            Toast.makeText(contexto, "Error de base de datos!", Toast.LENGTH_SHORT).show();
            throw new RuntimeException(e);
        }
    }
}
