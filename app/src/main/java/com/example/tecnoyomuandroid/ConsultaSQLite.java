...
public class ConsultaSQLite {
    ...
    public List<Empleado> ConsultarEmpleadosNoDisponiblesUsuarioSQLite() {
        try {
            List<Empleado> listaAuxiliar = new ArrayList<>();
            CrearBDSQLite manejadorBD = new CrearBDSQLite(context, "RegistroTecnoyomu", null, 1);
            SQLiteDatabase baseDatos = manejadorBD.getWritableDatabase();
            ...
            if (empleados.moveToFirst()) {
                ...
                Toast.makeText(context, "No hay empelados ligados a usuarios", Toast.LENGTH_SHORT).show();
                baseDatos.close();
                return listaAuxiliar;
            }
        } catch (Exception e) {
    ...
    public List<Equipo> ConsultarEquiposPorRepararSQLite() {
        try {
            ...
            SQLiteDatabase baseDatos = manejadorBD.getWritableDatabase();
            //Obtenemos los datos de la base de datos
            Cursor equipos = baseDatos.rawQuery("select * from equipo where estado='ingresado'", null);
            //Verificar que la consulta tenga datos
            if (equipos.moveToFirst()) {
                //Obtenemos todos los datos de la tabla
                do {
